...
**Important:** If the learner provided architecture docs during `/onboard`, those take precedence over the patterns in this file. Use this file as a fallback when no architecture docs are provided, or to supplement architecture docs that are incomplete.
...
- Still validate choices against the learner's experience level
...
- Propose whichever fits the learner's project and experience level
...
If the learner wants to deploy the app for a live link, discuss options appropriate to their stack (Vercel, Netlify, GitHub Pages, Railway, Fly.io, etc.) and make sure the architecture supports easy deployment. Verify that deployment config uses environment variables for secrets, enforces HTTPS, and has intentional CORS settings. A deployed link strengthens the project but isn't required.
...
- **Optional demo video:** The learner can record a short video and upload to YouTube or Vimeo. Not required, but it can strengthen the submission.
