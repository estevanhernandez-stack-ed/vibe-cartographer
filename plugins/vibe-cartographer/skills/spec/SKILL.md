...
- **Architecture self-review:** Step back and check your own work. Use subagents if available (**Dispatch tier:** `judgment` — adversarial self-review is verdict work; the model's findings are the deliverable, so it stays on the session model and never downgrades). Check for: ambiguities /build would stumble on, PRD stories without a clear home in the architecture, complexity that doesn't fit the project scope, consistency between data model, file structure, and components. Surface 2-3 findings as genuine questions for the builder.
- **Subsystem config pattern-match (when extending existing infra):** When the spec adds new code to an existing subsystem with established config conventions — Cloud Functions (memory, timeout, region), MCP server tools (auth scope, schema shape), Firestore writers (transaction patterns, version fields), .NET csproj chains (per `Build-config hygiene` in `/build`), Rust workspace profiles, etc. — the agent grepping existing values and using established floors as defaults is a load-bearing discipline. The vibe coder doesn't always have stack-specific mastery (Firebase Functions memory floors, Eventarc region constraints, Octokit auth wiring), and shouldn't have to. Architecture self-review explicitly checks: *"For every new file added to an existing subsystem, did I grep at least one sibling file's config values and use them as the floor?"* If not, surface as a finding — the builder may want to override, but the default should be "match the established pattern, then deviate intentionally."
