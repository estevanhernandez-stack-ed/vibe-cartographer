...
If the user explicitly asks to log the run and a decision-log MCP is available, append a one-line decision there with `category: architecture` (since voice profile changes are architectural for how the agent operates) — auto-detects the 626Labs dashboard (`mcp__626labs-cloud__manage_decisions`) when present. No decision-log MCP, or no explicit ask? No logging. The MCP is never required. This optional/auto-detect framing follows the [family decision-log convention](https://github.com/estevanhernandez-stack-ed/vibe-plugins/blob/main/docs/conventions/decision-log-backend.md).
