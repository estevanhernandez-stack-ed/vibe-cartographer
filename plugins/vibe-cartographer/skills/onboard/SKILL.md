...
**Order at command start (once all pieces are wired): session-logger.start() → decay check → rest of onboard.** The session-logger sentinel must be on disk before the decay check runs so `friction-logger.detect_orphans()` can pair a sentinel to a terminal for this run.
...
- **"(b)", "autonomous", "just go", "yolo", anything affirming (b)** → autonomous. **From this point on, the run flows end-to-end without further pauses.** Once the builder has opted in at this single gate, do NOT ask any more questions for the rest of `/onboard` — no per-step confirmations, no "still good?" check-ins, no embedded decay confirmations (any stale `_meta` field is deferred and surfaces again on the next interactive run; do not stamp it). Skip the entire interview (steps 3 through 11b) and go straight to artifact generation (steps 12 and 13). Use this fill-strategy:
  - **Returning-builder branch:** pull every value from the unified profile (`~/.claude/profiles/builder.json`). For any field that's missing or marked stale, use the defaults table below.
  - **New-builder branch:** the profile is mostly empty. Apply these defaults — `name: "Builder"` (placeholder), `technical_experience.level: "intermediate"`, `technical_experience.languages: []`, `technical_experience.frameworks: []`, `mode: "builder"`, `persona: null` (system default), `tone: "casual, direct"`, `pacing: "brisk"`, `creative_sensibility: "Clean and functional. Refine in /scope or /reflect."`, `project_origin: "blank folder (assumed)"`, `project_goals: "to be confirmed in /scope"`, `design_direction: "no strong signals — default to clean and functional"`, `deployment_target: null`. The builder-profile-portability hint still emits (information-only, no pause).
  - **Both branches:** generate `docs/builder-profile.md` and update the unified profile per steps 12-13. Surface every value used (whether pulled from profile or defaulted) in `docs/builder-profile.md` *and* in a new `## /onboard — autonomous run` section of `process-notes.md`, marking each defaulted value with `(default — confirm on next run)`. Then hand off to `/scope` per the existing Handoff section.

  **The contract is: opt in once at the pacing question, then Cart gets out of the way and produces artifacts. Visibility is the contract — every assumption is recorded — but not pausing is also the contract. Don't ask for confirmation. Flow.** Builders who want to refine just re-run `/onboard` interactive on the next pass.
