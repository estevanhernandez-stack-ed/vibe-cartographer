...
> **Disposition: [applied — pending Este's review of the contract text before push] 2026-06-09** — committed locally (not pushed; version unbumped). Re-landed as a single guide section "Autonomy Mode Adaptation" + one pointer line in each of the 9 chain command SKILLs + the evolve-cart Plugin-track guard in §5. **Reconstruction call:** the lost `three-modes.md` is reconstructed as the 2×2 grid of `autonomy_level` (guided | fully-autonomous) × `cycle_builder_identity` (self | agent-persona), collapsed to **three modes** because the two `guided` cells are behaviorally identical — which also explains why the original `three-modes.md` superseded `four-modes.md`. The full contract text appears verbatim in the run report for review; per the ratification condition it merges only after Este reads it (the push gate enforces this). No changes to the persona or Learner/Builder tables (autonomy is the third axis).
...
> **Disposition: [applied] 2026-06-09** — committed locally (not pushed; version unbumped). evolve-cart §6 now mandates writing/updating `proposed-changes.md` at the solo-repo root (observations + per-proposal disposition + exact diff for applied Plugin-track edits) and appends the "Plugin-track edits applied this run are UNCOMMITTED" line to the summary template. New `/vitals` Check #10 "Evolve continuity" verifies every `evolve*` session entry's `applied_files` still exist (lost-vs-superseded disambiguated via `proposed-changes.md`). Reconciled the pre-existing 8-vs-9 check-count drift to ten throughout the SKILL + Mental Trace (added the missing Check #9 trace line too) and brought `commands/vitals.md` into parity (10 checks listed + corrected its stale "never writes" claim, since the SKILL has six auto-fixes).
