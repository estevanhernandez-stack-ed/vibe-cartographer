...
| `/evolve-cart` | **L3:** Plugin reads its own session logs and proposes self-improvements        |
...
The unified profile is part of the [Self-Evolving Plugin Framework](docs/self-evolving-plugins-framework.md) — see that doc for the thesis, 12-pattern catalog, and the maturity ladder the plugin is working through. Vibe Cartographer is now at **Level 3** (reflective evolution via `/evolve-cart`): the plugin reads its own session logs, surfaces patterns across your runs, and proposes concrete SKILL edits for you to approve. Nothing auto-applies — every change is one-at-a-time, with consent.
